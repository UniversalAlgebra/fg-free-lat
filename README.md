...
**Theorem.**
Suppose $\mathbf L = \langle L, \wedge, \vee\rangle$ is a finite lattice and $h\colon \mathbf{F} \rightarrow \mathbf{L}$ a lattice epimorphism.
...
$$
h x \leq y \quad \Leftrightarrow \quad x \leq \alpha y,
$$
$$
y \leq h x \quad \Leftrightarrow \quad \beta y \leq x.
$$
...
$$
We claim that $G$ generates $\ker h$.  To prove this, we first show, by induction on term complexity, that for every $y \in L$, for every $r \in h^{-1}\{y\}$, the pairs $(r,\alpha y)$ and $(r,\beta y)$ belong to the sublattice $\langle G \rangle \leq \mathbf F \times \mathbf F$ generated by $G$.
...
Therefore, 
$$
\beta y \leq \beta h(s) \vee \beta h(t) \leq \alpha {h(s)} \vee \alpha {h(t)} \leq \alpha y.
$$


Also, $r \leq \alpha y$, so $r = \alpha y \wedge (s\vee t)$.  Taken together, these observations yield
$$
...
Similarly, $(r, \alpha y) \in \langle G \rangle$.  Indeed, $\beta y \leq r$ implies $r = \beta y \vee s\vee t$, and $\beta {h(s)} \vee \beta {h(t)} \leq \alpha y$ implies $\alpha y = \alpha y \vee \beta {h(s)} \vee \beta {h(t)}$. Therefore,
...
$$
\beta y \leq \beta h(s) \wedge \beta h(t) \leq \alpha {h(s)} \wedge \alpha {h(t)} \leq \alpha y.
$$
Also, $\beta y \leq r \leq \alpha y$ so $r = \alpha y \wedge s\wedge t$
...
**Claim 1:** There exists $N<\omega$ such that for all $(p_i, q_i)$ in $K$, if $p_i \geq x_0$, then $q_i \geq \beta_N (y_0)$.
...
Since $K$ is finite, we can find such $N_i$ for each $(p_i,q_i) \in K$. Let $N = \max\{N_i : 1 \leq i \leq m\}$.
Then for all $1\leq i \leq m$ the following implication holds:

$(\star)  \qquad \qquad \qquad \qquad \qquad p_i \geq x_0 \quad \Longrightarrow \quad q_i \geq \beta_N(y_0).$

**Claim 2:** There exists $N < \omega$ such that, for all $(p, q) \in \langle K \rangle$,

$(\ast) \qquad \qquad \qquad \qquad \qquad p \geq x_0 \quad \Longrightarrow \quad q \geq \beta_N(y_0).$

<!-- we have
$$
(\ast) \qquad \qquad \text{if } p \geq x_0, \text{ then $q \geq \beta_N(y_0)$}
$$ -->
...
*Case 1:* Assume $(p, q) = (p_1, q_1) \wedge (p_2, q_2)$, where $p_i$, $q_i$ ($i = 1, 2$) satisfy (*). Assume $p\geq x_0$.<!-- (We show $q \geq \beta_N(y_0)$.)--> Then $p = p_1 \wedge p_2 \geq x_0$, so $p_1 \geq x_0$ and $p_2 \geq x_0$, so (by the induction hypothesis) $q_1\geq \beta_N(y_0)$ and $q_2\geq \beta_N(y_0).$ Therefore, $q = q_1 \wedge q_2 \geq \beta_N(y_0),$ as desired.

*Case 2:* Assume $(p, q) = (p_1, q_1) \vee (p_2, q_2)$, where $p_i$, $q_i$ ($i = 1, 2$) satisfy (*). Assume $p\geq x_0$.<!-- (We show $q \geq \beta_N(y_0)$.)--> Then $p = p_1 \vee p_2 \geq x$.  Since $x_0$ is a generator, it is join prime in $\mathbf{F}(X)$, so either $p_1 \geq x_0$ or $p_2 \geq x_0$.  Assume (wlog) $p_1 \geq x_0$. Then, (by induction hypothesis) $q_1\geq \beta_N(y_0).$
Therefore, $q = q_1 \vee q_2 \geq q_1 \geq \beta_N(y_0),$ as desired.
...
**Theorem.**
Let $h\colon \mathbf{F} \rightarrow \mathbf{M_3}$ be the epimorphism that acts on the generators as follows: $x\mapsto a$, $y\mapsto b$, $z\mapsto c.$ Then $\operatorname{ker} h$ is not finitely generated.
<!--If $K = \operatorname{ker}\left(\mathbf{F}\{x, y, z\} \rightarrow \mathbf{M_3}\right)$, then $K$ is not finitely generated.   -->
...
Fix $u\in \{0, x, y, z\}$. Since $X$ is finite, [Lemma 2](#lemma-2) implies that there exists $M \in \mathbb{N}$ such that for every $(p, q) \in X$ with $p, q \in C_u$, we have $p, q \leq m_{u, M}$.

*Claim.* For $(p,q) \in \langle X \rangle$ and $u \in \{x, y, z\}$, the following implication holds:

$(\star) \qquad \qquad \qquad \qquad q \leq u \quad \Longrightarrow \quad p\leq m_{u, M}$.

We prove the claim by induction on the complexity of terms.
Fix $(p,q) \in \langle X \rangle$. Then $p, q \in C_u$ for some $u\in \{x, y, z\}$.

+ *Case 0.* Suppose $(p, q) \in X$. Then by definition of $M$ we have $p, q \leq m_{u, M}$.

+ *Case 1.* Suppose $(p, q) = (p_1, q_1) \wedge (p_2, q_2)$, where 
  $(p_i, q_i)$ satisfies (\star) for $i = 1, 2$.    
  If $q= q_1 \wedge q_2 \leq u$, then, since generators in the free lattice are meet-prime (see Theorem 1.11 below), we have $q_1\leq u$ or $q_2\leq u$. Assume $q_1\leq u$.  Then, by the induction hypothesis, $p_1\leq m_{u, M}$.  Therefore, $p = p_1\wedge p_2 \leq m_{u, M}$, as desired.

+ *Case 2.* Suppose $(p, q) = (p_1, q_1) \vee (p_2, q_2)$, where 
  $(p_i, q_i)$ satisfies (\star) for $i = 1, 2$.    
  If $q= q_1 \vee q_2 \leq u$, then 
  $q_i \leq u$ for $i = 1, 2$.
  It now follows from the induction hypothesis that $p_i\leq m_{u, M}$ for $i = 1, 2$, so 
  $p = p_1 \vee p_2 \leq m_{u, M}$, as desired.
...
+ *Case:* $u \in \{x, y, z\}$.    
  ...
  *Subclaim.* For all $n\in \mathbb N$,
  ...
  $m_{x,n} \geq m_{y,n} \wedge m_{z,n}$.

  So, by way of contradiction, suppose 

  (‡)  $\qquad m_{x,n} \geq m_{y,n} \wedge m_{z,n}$.

  Now, $m_{y,n} = y \vee (x \wedge z) \vee \cdots$, so clearly $m_{y,n} \geq y$.  Similarly, $m_{z,n} \geq z$.
  This, together with (‡), implies $m_{x,n} \geq m_{y,n} \wedge m_{z,n} \geq y \wedge z$.  But then Theorem 1.11 below implies that either $m_{x,n} \geq y$ or $m_{x,n} \geq z$, which contradicts Subclaim 2 above.
...
+ *Case:* $u = 0$.  
  ...
  Clearly 
  $x\wedge y \leq (x \vee (y\wedge z))\wedge (y \vee (x\wedge z))$. Suppose $x\wedge y = (x \vee (y\wedge z))\wedge (y \vee (x\wedge z))$. Then $(x \vee (y\wedge z))\wedge (y \vee (x\wedge z))\leq x$.
  By Theorem 1.11, the latter holds iff
  $x \vee (y\wedge z)\leq x$ or
  $y \vee (x\wedge z)\leq x$
  The first of these inequalities is clearly false, so it must be the case that $y \vee (x\wedge z)\leq x$.  But then $y \leq x$, which is obviously false.  We conclude that
...
##### Lemma 2
For each $u \in \{x, y, z\}$, for each $p \in C_u \cup C_0$, there exists $n \in \mathbb N$ such that $p\leq m_{u,n}$.  
...
+ *Case:* $p = p_1 \vee p_2$.  
  ...
  the induction hypothesis yields $i$ and $j$ for which $p_1 \leq m_{x, i}$ and $p_2 \leq m_{x, j}$. Letting $n = \max\{i, j\}$, we have 
  $p_1, p_2 \leq m_{x, n}$, from which 
  $p = p_1 \vee p_2 \leq m_{x, n}$, as desired.

+ *Case:* $p = p_1 \wedge p_2$.  
  If $p \in C_x$, then we may assume $p_1 \in C_x$ and $p_2 \in C_x \cup C_0$. By the induction hypothesis, there exists $n\in \mathbb N$ such that $p_1 \leq m_{x, n}$, whence $p \leq p_1 \leq m_{x,n}$.

  If $p \in C_0$, then each $p_i$ belongs to $C_u \cup C_0$ for some $u\in \{x, y, z\}$.  If $p_1 \in C_x \cup C_0$, then $p_1 \leq m_{x, n}$, as above and we're done.  Similarly, if $p_2 \in C_x \cup C_0$.  So assume $p_1 \in C_y \cup C_0$ and $p_2 \in C_z \cup C_0$. Then the induction hypothesis implies that there exist $i$ and $j$ such that 
  $p_1 \leq m_{y, i}$ and $p_2 \leq m_{z, j}$. If $n = \max\{i, j\}$, then
  $p_1 \leq m_{y, n}$ and $p_2 \leq m_{z, n}$.  Then, by the above definition of the sequences, we have
  $p_1 \wedge p_2 \leq m_{y, n} \wedge m_{z, n} \leq m_{0,n} \leq m_{x,n+1}$.
...
Then the kernel of an epimorphism $h\colon \mathbf{F} \rightarrow \mathbf{L}$ is a finitely generated sublattice of $\mathbf{F} \times \mathbf{F}$.
+ **Ex 2.** Let $X = \{x, y, z\}$ and let $\mathbf{L} = \mathbf{3}$ be the 3-element chain.    
Then the kernel of an epimorphism $h\colon \mathbf{F} \rightarrow \mathbf{L}$ is finitely generated.
+ **Ex 4.** Let $n > 2$, $X = \{x_0, x_1,\dots, x_{n-1}\}$, and $\mathbf{L} = \mathbf{2} \times \mathbf{2}$.  
Let $h\colon \mathbf{F} \rightarrow \mathbf{L}$ be an epimorphism. Then $K = \operatorname{ker}h$ is finitely generated.  
...
+ *Case:* $r \in X_a$.  
   Then $r\geq \bigwedge X_a \geq \bigwedge (X_a\cup X_1) = M(a)$.

+ *Case:* $r = s \vee t$, where $h(s) = a$ and $h(t) \in \{a, 0\}$.  
Assume (the induction hypothesis) that $s \geq M(a)$. Then 
$r = s\vee t \geq M(a)$.

+ *Case:* $r = s \wedge t$, where $h(s) = a$ and $h(t) \in \{a, 1\}$.  
Assume (the induction hypothesis) that $s, t \geq M(a)$. 
Then $s \wedge t \geq M(a)$.

This proves that for each $r \in h^{-1}\{a\}$ we have $r \geq M(a)$, and 
...
$g_a \wedge g_b \in h^{-1}\{0\}$. It remains to prove that $r \leq g_a \wedge g_b$ holds for all $r \in h^{-1}\{0\}$.

Fix $r \in h^{-1}\{0\}$. Then $h(r \vee g_a) = h(r) \vee h(g_a) = 0 \vee a = a$, which places $r \vee g_a$ in $h^{-1}\{a\}$.  Therefore, 
by maximality of $g_a$, we have $r \vee g_a  \leq g_a$, whence 
$r\leq g_a$.  Similarly, $r\leq g_b$.
...
$\ell_a \vee \ell_b \in h^{-1}\{1\}$. It remains to prove that $r \geq \ell_a \vee \ell_b$ holds for all $r \in h^{-1}\{1\}$.

Fix $r \in h^{-1}\{1\}$. Then $h(r \wedge \ell_a) = h(r) \wedge h(\ell_a) = 1 \wedge a = a$, which places $r \wedge \ell_a$ in $h^{-1}\{a\}$.  Therefore, 
by minimality of $\ell_a$, we have $r \wedge \ell_a  \geq \ell_a$, whence 
$r\geq \ell_a$.  Similarly, $r\geq \ell_b$.
...
+ *Subcase 1.2.*   
  $h(r) = a$ implies (wlog) $h(s) = a$ and $h(t) \in \{a, 1\}$.  Assume (the induction hypothesis) that $(s, \ell_a), (s, g_a), (t, \ell_p), (t, g_p)$ belong to $\langle Y \rangle$. By Claim 1, $\ell_a \leq \ell_1$, so $\ell_a = \ell_a \wedge \ell_1$.
...
**Corollary 1.5.** Let $\mathcal V$ be a nontrivial variety of lattices and let $\mathbf{F}_{\mathcal V}(X)$ be the relatively free lattice in $\mathcal V$ over $X$.  For each finite nonempty subset $S \subseteq X$, $\bigwedge S$ is join prime and $\bigvee S$ is meet prime. In particular, every $x\in X$ is both join and meet prime.  Moreover, if $x\leq y$ for $x, y \in X$, then $x = y$.
...
(W)  If $v = v_1 \wedge \cdots \wedge v_r \leq u_1 \vee \cdots \vee u_s = u$, 
then either $v_i \leq u$ for some $i$, or $v \leq u_j$ for some $j$. 
...
\wedge x_n \leq u_1 \vee \cdots \vee u_s \vee
y_1 \vee \cdots \vee y_m = u$, where $x_i, y_j\in X$, then either 
$x_i = y_j$ for some $i$ and $j$, or $v_i \leq u$ for some $i$, or
$v \leq u_j$ for some $j$. 
...
($\ast$) $\quad s^{\mathbf{F}(X)} \leq t^{\mathbf{F}(X)}$
...
$s_i^{\mathbf{F}(X)} \leq t^{\mathbf{F}(X)}$ for all $i$.
3. If $t = t_1 \wedge \dots \wedge t_k$ is a formal meet, then ($\ast$) holds iff 
$s^{\mathbf{F}(X)} \leq t_i^{\mathbf{F}(X)}$ for all $i$.
4. If $s = x_i$ and $t = t_1 \vee \dots \vee t_k$ is a formal join, 
   then ($\ast$) holds iff $x_i \leq t_j^{\mathbf{F}(X)}$ for some $j$.
5. If $s = s_1 \wedge \dots \wedge s_k$ is a formal meet and $t = x_i$, then ($\ast$) holds iff $s_j^{\mathbf{F}(X)} \leq x_i$ for some $j$.
6. If $s = s_1 \wedge \dots \wedge s_k$ is a formal meet and 
and $t = t_1 \vee \dots \vee t_m$ is a formal join, then ($\ast$) holds iff 
$s_i^{\mathbf{F}(X)} \leq t^{\mathbf{F}(X)}$ for some $i$
or $s^{\mathbf{F}(X)} \leq t_j^{\mathbf{F}(X)}$ for some $j$
...
there exists $c\in A$ such that $a\leq c$ and $b\leq c$.  A lattice is *upper continuous* if whenever $A\subseteq L$ is an up directed set having a least upper
...
If $\mathbf K = \langle K, \vee, \wedge\rangle$ is a lattice generated by a finite set $X$, then we can write $K$ as a union of a chain of subsets $H_0\subseteq H_1 \subseteq \cdots$ defined inductively by setting $H_0 = X^\wedge$ and $H_{k+1} = (H_k)^{\vee \wedge}$, for all $k\geq 0$. By induction, each $H_n = X^{\wedge(\vee\wedge)^n}$ is a finite meet-closed subset of $K$, and $\bigcup H_n = K$, since $X$  generates $\mathbf X$.

Let $h \colon \mathbf K \to \mathbf L$ be a lattice epimorphism and, for each $y \in L$ and $k< \omega$, define
$$
\beta_k(y) = \bigwedge \{w \in H_k : h(w) \geq a\}.
$$
...
"...[$h$ is lower bounded] if and only if for each $a \leq h(1_{\mathbf K})$ there exists $N\in \omega$ such that $\beta_n(a) = \beta_N(a)$ for all $n\geq N$."
