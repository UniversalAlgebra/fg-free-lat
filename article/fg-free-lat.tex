...
Then there exists $N>0$ such that for all $(p,q) \in \<K\>$, if $p\geq \beta_k h(p)$ and 
$p\ngeq \beta_{k-1}h(p)$, then $q \geq \beta_{k+N}h(q)$.
\end{lemma}
...
\begin{proof}
Since $K$ is finite, it should be obvious that we can find an $N>0$ such that 
