...
the {\bf length} of $t$, denoted $\len{t}$, as follows:
\begin{itemize}
\item $t$ has \emph{length} $k$ iff $t \in H_{k} \setminus H_{k-1}$
\item $t$ has \emph{length at most} $k$ if $t \in H_k$.
\end{itemize} 
Observe that if $t$ has length at most $\ell$, that is, $\len{t} \leq k$,
then $t\geq \beta_k h (t)$.
Also, it is possible to have $\len{p_1 \wedge p_2} < \min\{\len{p_1},\len{p_1}\}$.
Consider the following example: let $x, x_1, \dots, x_n, y, y_1, \dots, y_m$ be distinct 
generators of $\alg F(X)$, let $a = t(x_1, \dots, x_n)$ be a term of length $j$
that does not involve $y$, and assume 
$a\geq x$ (e.g., $a = x \vee (x_1 \wedge x_2)$ has length $1$).
Let $b = s(y_1, \dots, y_m)$ be a term of length $k$ that does not involve $x$ 
and assume $b \geq y$ (e.g., $b = y \vee (y_1 \wedge y_2)$ has length $1$).
Then 
$p_1 = a \wedge y$ has length at least $j$ and 
$p_2 = b \wedge x$ has length at least $k$, while
\[
p_1 \wedge p_2 =
...
Suppose $(p,q) \in \ker h$ and $a$ and $b$ are as above and suppose 
\[
\binom{p}{q} = \binom{a}{q_1} \wedge \binom{b}{q_2} = \binom{a \wedge b}{q_1 \wedge q_2}
=\binom{x \wedge y}{q_1 \wedge q_2} = 
\binom{x }{q_1} \wedge  \binom{y}{q_2}.
\]
\newpage
...
Suppose 
\[
K = \{(x, x), (y, y), (z,z), (x, y\wedge z), (y, x\wedge z), (z, x\wedge y),
(y\wedge z, x), (x\wedge z, y), (x\wedge y, z)\}.\]
Then 
\[
\binom{x}{x} \vee \binom{y \wedge z}{x}  = \binom{x \vee (y \wedge z)}{x} 
= \binom{x_1}{x_0} \in \<K\>.
\]

% Suppose $(y, y) \in K$ and $(y, x\wedge z) \in K$. Then 
\[
\binom{y}{y} \vee \binom{x \wedge z}{y}  = \binom{y \vee (x \wedge z)}{y}
= \binom{y_1}{y_0} \in \<K\>.
\]

% Suppose $(z, z) \in K$ and . Then 
\[
\binom{z}{z} \vee \binom{x \wedge y}{z}  = \binom{z \vee (x \wedge y)}{z}
= \binom{z_1}{z_0} \in \<K\>.
\]

Therefore,
\[
\binom{x}{x} \vee \left[\binom{y_1}{y} \wedge \binom{z_1}{z}\right] 
= \binom{x\vee (y_1 \wedge z_1)}{x  \vee (y \wedge z)}
= \binom{x_2}{x_1} \in \<K\>.
\]
...
\[
\binom{y}{y} \vee \left[\binom{x_1}{x} \wedge \binom{z_1}{z}\right] 
= \binom{y\vee (x_1 \wedge z_1)}{y  \vee (x \wedge z)}
= \binom{y_2}{y_1} \in \<K\>.
\]
...
\[
\binom{z}{z} \vee \left[\binom{x_1}{x} \wedge \binom{y_1}{y}\right] 
= \binom{z\vee (x_1 \wedge y_1)}{z  \vee (x \wedge y)}
= \binom{z_2}{z_1} \in \<K\>.
\]
...
\[
\binom{x}{x} \vee \left[\binom{y_2}{y_1} \wedge \binom{z_2}{z_1}\right] 
= \binom{x\vee (y_2 \wedge z_2)}{x  \vee (y_1 \wedge z_1)}
= \binom{x_3}{x_2} \in \<K\>.
\]

If we had $(y_1, x), (z_1, x) \in \<K\>$, then 
\[
\binom{x}{x} \vee \left[\binom{y_1}{x} \wedge \binom{z_1}{x}\right] 
= \binom{x\vee (y_1 \wedge z_1)}{x  \vee (x \wedge x)}
= \binom{x_2}{x} \in \<K\>.
\]

If we had $(y_1, z), (x_1, z) \in \<K\>$, then 
\[
\binom{z}{z} \vee \left[\binom{y_1}{z} \wedge \binom{x_1}{z}\right] 
= \binom{z\vee (x_1 \wedge y_1)}{z  \vee (z \wedge z)}
= \binom{z_2}{z} \in \<K\>.
\]

\medskip
\[
\binom{y}{x\wedge z} \vee \left[\binom{x_2}{x} 
\wedge \binom{z_2}{z}\right]  = 
\binom{y\vee (x_2\wedge z_2)}{(x\wedge z) \vee (x \wedge z)}
= \binom{y_3}{x\wedge z}
\]


\[
\binom{y_1}{x} = \binom{y \vee (x \wedge z)}{x}
\]
\bigskip

\[
\binom{x}{x} \vee \binom{x}{y \wedge z}  = \binom{x}{x \vee (y \wedge z)} 
= \binom{x}{x_1} \in \<K\>.
\]
...
Suppose $p \geq \beta_1(a)$ and $p \ngeq \beta_0(a)$ and $(p,q) \in \<K\>$ and 
\[
...
So, 
\[
p_1 \vee p_2 \geq \beta_1(a) = \bigwedge \{w \in H_1 \mid h(w) \geq a\}.
\]
Let's consider a simple example and see how the induction would go through. Suppose,
\[
\beta_1(a) = \bigl[(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4)\bigr] \wedge \bigl[(x_3 \wedge x_5)\vee x_1\bigr]\leq p_1 \vee p_2.
\]
By (W), one of the following holds:
\begin{enumerate}
  \item $(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4) \leq p_1 \vee p_2$;
  \item $(x_3 \wedge x_5)\vee x_1 \leq p_1 \vee p_2$;
  \item $\bigl[(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4)\bigr] \wedge \bigl[(x_3 \wedge x_5)\vee x_1\bigr] \leq p_1$;
  \item $\bigl[(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4)\bigr] \wedge \bigl[(x_3 \wedge x_5)\vee x_1\bigr]\leq p_2$.
\end{enumerate}}
The firt two are impossible. Indeed, Case (1) implies
$x_2 \wedge x_4 \leq p_1 \vee p_2 = p$, but $x_2 \wedge x_4 \in H_0$, so we would have
$\beta_0(a) \leq x_2 \wedge x_4 \leq p$, contradicting the hypothesis $\beta_0(a)\nleq p$.
Similarly, in the second case we reach the contradiction 
$\beta_0(a) \leq x_1 \leq p_1 \vee p_2 = p$.
In case (3) or (4), we have $\beta_1(a) \leq p_1$ or $\beta_1(a) \leq p_2$, respectively.
Therefore, by the induction hypotheses, $\beta_N(a)\leq q_1$ or $\beta_N(a)\leq q_2$.
Either way, $q = q_1 \vee q_2 \geq \beta_N(a)$, as desired.

Next suppose $p = p_1 \wedge p_2 \geq \beta_1(a)$.
Then we have both $p_1 \geq \beta_1(a)$ and $p_2 \geq \beta_1(a)$, so the induction hypothesis yields
$q_1 \geq \beta_N(a)$ and $q_2 \geq \beta_N(a)$, so $q = q_1 \wedge q_2 \geq \beta_N(a)$.

Since the case $(p,q) = (p_1, q_1) \wedge (p_2, q_2)$ is easy no matter what form $\beta_k(a)$ takes,
let's consider the $\vee$ case more generally.

Suppose $p \geq \beta_k(a)$ and $p \ngeq \beta_{k-1}(a)$ and $(p,q) \in \<K\>$ and 
\[
\binom{p}{q} = \binom{p_1}{q_1} \vee \binom{p_2}{q_2},
\]
where $(p_i, q_i) \in \<K\>$ and assume (the induction hypothesis) that $p_i \geq \beta_j(a)$ 
implies $q_i \geq \beta_{j+N}(a)$.
We have 
\[
p = p_1 \vee p_2 \geq \beta_k(a) = \bigwedge \{w \in H_k \mid h(w) \geq a\}.
\]
and
\[
...
% \begin{align*}
% H_0 &= X^\wedge = \{\bigwedge B \mid B \finsub X\}\\
% H_k &= (H_{k-1})^{\vee \wedge} = \{\bigwedge B \mid B \finsub H_{k-1}^{\vee}\}
% \end{align*}
Recall,
\[
H_k = (H_{k-1})^{\vee \wedge} = \{\bigwedge B \mid B \finsub H_{k-1}^{\vee}\},
\]
so $w \in H_k$ means $w = \bigwedge B$ for some finite set $B$ containing 
joins of elements of $H_{k-1}$, and $p = p_1 \vee p_2$ is above a meet of the 
elements of $B$.
Therefore Whitman's condition implies that either $p_i \geq \beta_k(a)$ for 
some $i \in \{1,2\}$,  or $p_1 \vee p_2 \geq w$ for some meetand $w$ of $\beta_k(a)$.  
In the former case, $q_i \geq \beta_{k+N}(a)$,
...
$p \geq u \geq \beta_{k-1}(a)$ contradicting our initial hypothesis.


\[
\beta_k(a) = \bigwedge \bigvee \bigl[(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4)\bigr] \wedge \bigl[(x_3 \wedge x_5)\vee x_1\bigr]\leq p_1 \vee p_2.
\]
By (W), one of the following holds:
\begin{enumerate}
  \item $(x_1 \wedge x_2 \wedge x_3) \vee (x_2 \wedge x_4) \leq p_1 \vee p_2$;
...
\end{itemize}
In other words, $m$ and $M$ are the minimum and maximum lengths of 
...
$\len{p} = \ell$ and $p = p_1 \vee p_2$, where $\len(p_1), \len(p_2) > \ell$.
...
at most $\max\{\ell_1, \ell_2\} + 1$. For each $i\in \{1,2\}$,
