...
$p \geq \beta_{k'}(a)$. As $k'$ is the smallest such number,
\wjd{consider the case $k' = 0$}
$p \ngeq \beta_{k'-1}(a)$, and since $k'\leq k$, we have
...
\begin{enumerate}[(a)]
  ...
  \item $p_1 \vee p_2 \geq w$ for some meetand $w$ of $\beta_{k}(a)$.    
\end{enumerate}
Assume Case (a) holds with $i=1$. Then
\[
p_1 \geq \beta_{k}(a)
= \bigwedge \{w \in H_{k} \mid h(w) \geq a\}.
\] 
...
\]
That is, $\beta_{k}(a) \geq \beta_{k}h(p_1)$.
Similarly, $\beta_{k}(a) \geq \beta_{k}h(p_2)$. Therefore, 
\[
p_1 \geq \beta_{k}(a)  \geq \beta_{k}h(p_1) \vee \beta_{k}h(p_2)
\geq \beta_{k}h(p_1),
\]
...
\[
p_i \geq \beta_{k}(a)  = \beta_{k}h(p_1 \vee p_2)
...
Similarly, $\beta_{k}(a) \geq \beta_{k}h(p_2)$. Therefore, 
\[
p_i \geq \beta_{k}(a)  \geq \beta_{k}h(p_1) \vee \beta_{k}h(p_2)
\geq \beta_{k}h(p_i),
\]
from which it follows, by the induction hypothesis, that 
