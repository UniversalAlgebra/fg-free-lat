...
\]
\[
\beta_{k+N}h(q_1 \vee q_2)
= \bigwedge \{w \in H_{k+N} \mid h(w) \geq h(q_1) \vee h(q_2)\}.
\] 
Clearly
\[
\{w \in H_{k+N} \mid h(w) \geq h(q_1) \vee h(q_2)\}
= \{w \in H_{k+N} \mid h(w) \geq h(q_1) \text{ and } h(w) \geq h(q_2)\},
\] 
which is equal to 
\[
\{w \in H_{k+N} \mid h(w) \geq h(q_1)\} \cap \{w \in H_{k+N} \mid h(w) \geq h(q_2)\},
\] 
so
\begin{align*}
  \bigwedge \{w \in H_{k+N} &\mid h(w) \geq h(q_1) \vee h(q_2)\}\\
&=\bigwedge \{w \in H_{k+N} \mid h(w) \geq h(q_1)\} \cap \{w \in H_{k+N} \mid h(w) \geq h(q_2)\}.
\end{align*}
...
We are interested in the property $\D(\alg{L}) =\alg L$, i.e., when every element is in
$\D_k(\alg L)$ for some \(k\). \emph{If $A$ is a finite subset of $\D_k(\alg L)$, 
then} $\bigvee A \in \D_{k+1}(\alg L)$. Indeed, if $\bigvee A \leq \bigvee S$ 
for some set \(S\), then, for $a \in A$, $S$ is a join cover of \(a\). 
Since \(a \in \D_k(\alg L)\), if this is a nontrivial join cover, 
there is a set $U_a \subseteq \D_{k-1}(\alg L)$ with $U_a \ll S$ and
$a \leq \bigvee U_a$. For those $a$'s for which $S$ is a trivial join cover, 
set $U_a = \{a\}$. Let $U = \bigcup U_a$.  Then $U \subseteq \D_k(\alg L)$
and is a join cover of $\bigvee A$ and $U \ll S$, which shows that $\bigvee A$ 
is in $\D_{k+1}(\alg L)$. Hence $\D(\alg L)$ is a join subsemilattice of $\alg L$. 
On the other hand, it is easy to see that in a lattice with the minimal join
cover refinement property, every element is a join of join irreducibles. 
Combining these observations, we obtain the following useful equivalence.

\begin{lemma}[\protect{\cite[Lem.~2.5.]{MR1319815}}]\label{lem:2.5}
For a lattice with the minimal join cover refinement property,
$\D(\alg L) = L$ if and only if $\J(\alg L) \subseteq \D(\alg L)$.
\end{lemma}

For example, if $\alg K$ is a finite distributive lattice, 
then $\D(\alg K) = \D_1(\alg K) = K$ because every join irreducible element 
is join prime. Likewise, it is easy to see that for the pentagon $\alg N_5$ we have 
$\D(\alg N_5) = \D_1(\alg N_5) = N_5$. On the other hand, for the
diamond we have $\D(\alg M_3) = \{0\} \neq M_3$.

We need to analyze the property $\D(\alg L) = L$ at length, but first let us show that
it does what we need it to do.

\begin{theorem}[\protect{\cite[Thm.~2.6]{MR1319815}}]\label{thm:2.6}
Let $\alg K$ be a finitely generated lattice and let $h\colon \alg K \to \alg L$ 
be a homomorphism. If $\D(\alg L) = L$, then $h$ is lower bounded.
\end{theorem}
...
\subsection{Join dependency relation}
Given a lattice $\alg{L}$, let $\J(\alg{L})$ denote the set of all join 
irreducible elements in $L$.
If $\alg{L}$ is a finite lattice and $u$ is a join irreducible element of $\alg{L}$, 
then \(u\) has a unique lower cover, which we will denote by \(u_\ast\). 
Dually, if \(p\) is meet irreducible in $\alg{L}$, then the unique upper cover 
of \(p\) is denoted by \(p^*\). Pairs of the form \((u, u_*)\) with
\(u \in \J(\alg{L})\) play a special role in the theory of congruences. 
% (We could equally well use the pairs \((p,p^*)\) with \(p \in \M(\alg{L})\).) 
This is reflected in a basic theorem due to R.~P.~Dilworth [40].

\begin{theorem}[\protect{\cite[Thm.~2.30]{MR1319815}}]\label{thm:2.30}
Let $\alg{L}$ be a finite lattice. A congruence $\theta$ is join irreducible in
$\Con \alg{L}$ if and only if $\theta = \Cg (u,u_\ast)$ for some 
$u \in \J(\alg{L})$. A congruence $\psi$ is meet
irreducible if and only if, for some $u \in J(\alg{L})$, $\psi$ is the unique 
largest congruence separating \(u\) and \(u_\ast \).
\end{theorem}

The correspondence between join irreducible congruences and join irreducible
elements of $\alg{L}$ is in general not one-to-one, e.g., as in $\alg{M}_3$. 

Clearly any congruence relation on a finite lattice
is determined by the covering pairs that it collapses. 
Assuming \(a \succ b\) and $a \rtheta b$
with $\theta \in \Con \alg{L}$, choose \(u\) minimal in $\alg{L}$ such that 
\(u \leq a\) and \(u \nleq b\). Then \(u \in \J(\alg{L})\),
and the interval $\llb u_\ast, u \rrb$ is projective with \(\llb b, a \rrb\) 
so in particular $u\rtheta u_\ast$. Thus every congruence
relation on a finite lattice is determined by the set of pairs 
\((u,u_\ast)\) with \(u \in \J(\alg{L})\)
that it collapses.

To analyze this connection further, we introduce 
the \emph{join dependency relation} \(D\)
on the set of elements of a (possibly infinite) lattice $\alg{L}$, defined by
% \begin{align*}
% a \rD b & \text{ if $a \neq b$, $b$ is join irreducible, and $\exists p \in L$ with} \\
% & a \leq b \vee p \text{ and $a \nleq c \vee p$ for all $c < b$}.
% \end{align*}
\[
a \rD b \; \text{ if $a \neq b \in \J(\alg{L})$ and } (\exists p \in L) (a \leq b \vee p \text{ and } \forall c < b, a \nleq c \vee p).
\]
Intuitively, $a \rD b$ if there exists $p$ such that $b$ is minimal with respect to $a \leq b \vee p$.
(See Figure~\ref{fig:2}.)  In this case we will say that $a \rD b$ via $p$, or $a$ ``depends on'' $b$ via $p$. 
If $b$ has a lower cover $b_\ast$, for example if $\alg{L}$ is finite, the last 
part of the definition can be replaced with $a \nleq b_\ast \vee p$. 
The join dependency relation is easily understood in terms of minimal nontrivial
join covers.
...
\begin{tikzpicture}[scale=\figscale]
  ...
  \node[lat] (bs) at (2,-1) {};
  \node[lat] (b) at (2,0) {};
  \draw[semithick] (p) -- (ba) -- (a) -- (aa) -- (b) -- (bs) -- (ba);
  \draw (a) node[left] {$a$};
  \draw (p) node[left] {$p$};
  \draw (bs) node[right] {$b_\ast$};
  \draw (b) node[right] {$b$};
\end{tikzpicture}
\caption{Example illustrating the relation $a \rD b$.}
  \label{fig:2}
\end{figure}
...
\begin{lemma}
  [\protect{\cite[Lem.~2.31]{MR1319815}}]\label{lem:2.31}
Let $\alg{L}$ be a lattice with the minimal join cover refinement property. 
For an element $a$ of $\alg{L}$ we have $a \rD b$ if and only if $b$ 
belongs to a minimal nontrivial join cover of $a$.
\end{lemma}

Let $\trianglelefteq$ denote the reflexive and transitive closure of $D$ restricted to 
$\J(\alg{L})$.
Then $\trianglelefteq$ is a quasiorder (reflexive and transitive), and so it 
induces an equivalence relation $\equiv$ on $\J(\alg{L})$, modulo which 
$\trianglelefteq$ is a partial order, {\it viz.}, $a \equiv b$ if and only if
$a \trianglelefteq b$ and
$b \trianglelefteq a$.

\begin{lemma}
    [\protect{\cite[Lem.~2.36]{MR1319815}}]\label{lem:2.36}
Let $\alg{L}$ be a finite lattice and let $a, b \in \J(\alg{L})$. 
Then we have $\Cg(a,a_\ast) < Cg(b,b_\ast)$ if and only if $a\trianglelefteq b$, 
i.e., either $a = b$ or there exist $c_1, \dots, c_{k-1} \in \J(\alg{L})$ such that
\[
a \rD c_1 \rD \cdots \rD c_{k-1} \rD b.
\]
\end{lemma}

By a $D$-sequence in a lattice $\alg{L}$, we mean a sequence 
$a_0, a_1, \dots, a_{n-1} \in \J(\alg{L})$ of 
join irreducible elements of $\alg{L}$ such that 
$a_i \rD a_{i+1}$ for all $i$, where $2 \leq n \leq \omega$. 
A $D$-sequence is infinite if $n = \omega$. 
The elements of a $D$-sequence are not required to be distinct
(although $a_i \neq a_{i+i}$ by the definition of $D$), so we define a 
$D$-cycle to mean a finite sequence 
$a_0, a_1, \dots, a_{n-1} \in \J(\alg{L})$ ($n\geq 2$) of 
join irreducible elements of $\alg{L}$ such that
$a_i \rD a_{i+i}$ for all $i < n$, where the subscripts are computed modulo $n$. 

A $D$-cycle can obviously be regarded as a special type of infinite $D$-sequence 
(even though the indexing is different).
\begin{theorem}
    [\protect{\cite[Thm.~2.38]{MR1319815}}]\label{thm:2.38}
\label{thm:2.38}  
A finitely generated lattice $\alg{L}$ is lower bounded if and only if
it contains no infinite $D$-sequence and satisfies the minimal join cover refinement
property.
\end{theorem}

For the present work, a key part of the proof of Theorem~\ref{thm:2.38} is the 
proof that if $\alg L$ is not lower bounded, then there is an infinite $D$-cycle.
This is proved as follows: assume that $\alg L$ satisfies the 
minimal join cover refinement property, but is not lower bounded so 
$\D(\alg L) \neq L$. Then, by Lemma~\ref{lem:2.5}, this implies $\J(\alg L)\nsubseteq \D(\alg L)$,
so we can choose a join irreductibe element that does not belong to $\D(\alg L)$;
let $b_0 \in \J(\alg L)- \D(\alg L)$ denote such an element.
Now for each $b \in \J(\alg L)$, the set $\mathcal M(b)$
(of all minimal nontrivial join covers of $b$) is finite.
So given $b_i \in \J(\alg L)- \D(\alg L)$, some minimal nontrivial join cover of
$b_i$ contains an element $b_{i+1} \in \J(\alg L) - \D(\alg L)$, and 
we have $b_i \rD b_{i+1}$ In this way we can construct an infinite $D$-sequence
$\{b_i : i < \omega\}$.

Of course, we are particularly interested in the finite case.
\begin{cor}
    [\protect{\cite[Cor.~2.39]{MR1319815}}]\label{cor:2.39}
  A finite lattice L is lower bounded if and only if it contains
no $D$-cycle.  
\end{cor} 
This corollary makes it easy to show that certain lattices are not lower bounded.
For example, if we let $a_i$ ($i < n$) denote the atoms of the 2-dimensional modular
lattice $\alg{M}_n$, then for $n \geq 3$ we have the $D$-cycle
\[
a_0 \rD a_1 \rD \cdots \rD a_{n-1} \rD a_0,
\]
so $\alg{M}_n$ is not lower bounded. 
