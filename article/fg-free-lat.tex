...
\begin{lemma}\label{lem:prelemmanary}
If $h$ is an epimorphism from $\alg F(X)$ onto a finite lattice $\alg L$ 
and if $h$ is not lower bounded, then there exists an unbounded
class of $\ker h$ containing a join prime element.
\end{lemma}
\begin{proof}
We will show  that if no unbounded class of $\ker h$ has a join prime element, 
then 
\end{proof}
...
By Lemma~\ref{lem:2.7} above, for all $w \in F(X)$ and $a\in \D_n(\alg L)$, we have
$h(w) \geq a$ if and only if $w \geq \beta_n(a)$. Therefore, 
if $a \in \D_{k-1}(\alg L)\subseteq  \D_{k}(\alg L)$, we have
\begin{equation}\label{eq:256}
p \geq \beta_{k}(a) \iff h(p) \geq a \text{ and }
p \ngeq \beta_{k-1}(a) \iff h(p) \ngeq a.
\end{equation}
By~(\ref{eq:ih}) 
and~(\ref{eq:contr}), this leads to the following contradiction:
\[
h(p) \geq a \text{ and } h(p) \ngeq a.
\]
So it must be the case that $a \notin \D_{k-1}(\alg L)$.

Recall that $\rho(a)$ denotes the least natural number such that $a \in \D_{\rho(a)}(\alg L)$,
and is undefined if $a\notin \D(\alg L) = \bigcup \D_n(\alg L)$.
First, consider the case in which $\rho(a) = r$ is defined.
So it must be the case that $a \notin \D_{k-1}(\alg L)$.
Since $a \notin \D_{k-1}(\alg L)$, it must be the case that 
$r \geq k$.  Then $p \geq \beta_k(a)\geq \beta_r(a)$, so $h(p)\geq a$.
Therefore, $h(q) \geq a$, so $q \geq \beta_r(a)$.
...
It follows that, for all $j < \omega$,
\[
\beta_{j+1}(a) 
    = \beta_0(a) \wedge \bigwedge_{S \in \mathcal C (a)} \bigvee_{s\in S} \beta_{j}(s) 
\leq \beta_0(a) \wedge \bigvee_{1\leq i\leq m} \beta_{j} h(u_i) 
\]
Question: do we have 
\[
p \geq \beta_0(a) \wedge \bigvee_{1\leq i\leq m} \beta_{k-2} h(u_i)?
\] 
If so, then 
\[
p \geq \beta_0(a) \wedge \bigvee_{1\leq i\leq m} \beta_{k-2} h(u_i) \geq \beta_{k-1}(a),
\] 
a contradiction.
\[
\beta_{k-2} h(u_i)  = \bigwedge \{w \in H_{k-2} \mid h(w) \geq h(u_i)\}
\]

\[p \geq u_1 \vee \cdots \vee u_m,\] 

so 

\[h(p) \geq h(u_1) \vee \cdots \vee h(u_m),\] 
...
\[
p \geq \bigwedge \{w \in H_{k} \mid h(w) \geq a\},
\]
\[
H_{k} = (H_{k-1})^{\vee \wedge} = \{\bigwedge B \mid B \finsub H_{k-1}^{\vee}\},
\]
...
\[
p \geq \bigwedge \{w \in \{\bigwedge B \mid B \finsub H_{k-1}^{\vee}\} \mid h(w) \geq a\},
\]
...
\[
p \ngeq \beta_{k-1}(a) = \bigwedge \{w \in \{\bigwedge B \mid B \finsub H_{k-2}^{\vee}\} \mid h(w) \geq a\}.
\]

\[
u_i \in H_{k-1} = \{ \bigwedge B \mid B \finsub H_{k-2}^\vee \}
\]

\[
p \geq u_1 \vee \cdots \vee u_m = \bigvee_{1\leq i\leq m} \bigwedge B_i  \; \text{ where all } \;
B_i \finsub H_{k-2}^\vee
\]

Fix $i \in \{1, \dots, m\}$. Then,
\[
p = p_1 \vee p_2 \geq u_i = \bigwedge B_i \; \text{ where all } \;
B_i \finsub H_{k-2}^\vee,
\]
so 
\[
p = p_1 \vee p_2 \geq (v_1 \vee \cdots \vee v_z) \wedge \cdots \wedge  (v'_1 \vee \cdots \vee v'_{z'}) \; \text{ where all } \;
v_j, v'_j \in H_{k-2},
\]
so, by Whitman's condition, either 
\begin{enumerate}[(a)]
  \item $p_1 \geq u_i$ or $p_2 \geq u_i$ or
  \item $p_1 \vee p_2 \geq v_1 \vee \cdots \vee v_z$ for some set of $v_j$'s in $H_{k-2}$.
\end{enumerate}
