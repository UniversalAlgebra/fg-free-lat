...
\title{Generating subdirect products of lattices}
...
\begin{theorem} \label{thm:fiber}
 Let $C$ be a fiber product of finitely generated lattices $A$ and $B$ with respect to
 homomorphisms $g\colon A\to D, h\colon B\to D$ onto finite $D$.

 Then $C$ is finitely generated if $D$ is upper and lower bounded.
\end{theorem}

\begin{proof}
 By definition
\[ C = \{ (a,b)\in A\times B \sts g(a) = h(b) \}. \]  
 Adapt the proof that $\ker h \leq F(X)^2$ is finitely generated if $h\colon F(X)\to D$ is bounded.
\end{proof}

\begin{remark}
 The converse implication in Theorem~\ref{thm:fiber} holds for $A=B=F(X)$ and $g=h$ by Lemma~\ref{lem:bka}.
 More generally, the proof can be adapted immediately for $A,B$ satisfying Whitman's condition and being
 generated by join prime elements and by meet prime elements.

 The converse does not hold in general:
 Nik's example describes a lower but not upper bounded epimorphism $h\colon A\to D$ for $A$ with $14$ generators,
 $D$ the subgroup lattice of $\mathbb{Z}_2^3$ (cf. projective plane) such that $\ker h$ is finitely generated. 
\end{remark}  


\section{Bounded Homomorphisms}

 Let $F(X)$ denote the free lattice over a finite set $X$. For $k\in\N$ define
\[ H_0 := X^\wedge,\ H_{k+1} := H_k^{\vee \wedge} \]
 the set of meets of elements of $X$, meets of joins of elements in $H_k$, respectively. Then
\[ H_0\subseteq H_1\subseteq \dots \text{ and } F(X) = \bigcup_{k\in\N} H_k. \]
 Fix an epimorphism $h\colon F(X)\to L$ onto a finite lattice $L$ for the remainder of the section.
 For $k\in\N$ and $a\in L$ define
\[ \beta_k(a) := \bigwedge \{ w\in H_k \sts h(w) \geq a \}. \]
 Note that for $k>0$ 
\[ \beta_k(a) := \bigwedge \{ w\in H_{k-1}^\vee \sts h(w) \geq a \}. \]

\begin{lemma} \label{lem:beta}
 Let $k\in\N$.
\begin{enumerate}
\item \label{it:mon}
 If $a\leq b$ in $L$, then $\beta_k(a)\leq \beta_k(b)$.
\item \label{it:Vbk}
 $\bigvee \{ \beta_k(a) \sts a\in A \} \geq \beta_{k+1}(\bigvee A)$ for all $A\subseteq L$. 
\end{enumerate}
\end{lemma}  

\begin{proof}
 \eqref{it:mon} is immediate. 
 For~\eqref{it:Vbk} note that by definition $\beta_k(a) \in H_k$ and $h(\beta_k(a))\geq a$ for all $a\in L$.
 Hence $w := \bigvee \{ \beta_k(a) \sts a\in A \}$ is in $H_k^\vee$ and 
\[ h(w) = \bigvee \{ h(\beta_k(a)) \sts a\in A \} \geq \bigvee A \]
 yields the claim.
\end{proof}



\begin{lemma} \label{lem:bka}
 For each finite subset $G$ of $\ker h$ there exists $N\in\N$ such that
\begin{equation} \label{eq:bka}
  \forall (p,q)\in \langle G\rangle\ \forall k\in\N\ \forall a\in L\colon p\geq\beta_k(a) \Rightarrow q\geq\beta_{k+N}(a).
\end{equation}
\end{lemma}

\begin{proof}
 First note that since $G$ and $L$ are finite, we have $N\in\N$ such that for all $(p,q)\in G, a\in L$ and
 $k\in\N$, the implication $p\geq\beta_k(a) \Rightarrow q\geq\beta_{k+N}(a)$ holds.
 We show~\eqref{eq:bka} for this specific $N$ by induction on the complexity of $(p,q)$ over the generating set $G$.
 In the following we assume that $(p,q)$ is not a generator.

 Case $(p,q) = (p_1,q_1) \wedge (p_2,q_2)$ for $(p_1,q_1),(p_2,q_2)\in\langle G\rangle$:
 Immediate from the induction assumption.

 Case $(p,q) = (p_1,q_1) \vee (p_2,q_2)$ for $(p_1,q_1),(p_2,q_2)\in\langle G\rangle$:
 Let $a\in L$. We use induction on $k\in\N$. For the base case $k=0$, assume
 $p\geq\beta_0(a)$. By definition $\beta_0(a) = \bigwedge W$ for $W := \{ x\in X \sts h(x) \geq a \}$.
 By Whitman's condition $p_1\vee p_2 \geq \bigwedge W$ yields $p_1\geq \beta_0(a)$ or $p_2\geq \beta_0(a)$ or
 $p\geq x$ for some $x\in W$. Since generators in $F(X)$ are join prime, the latter case yields $p_1\geq x$ or
 $p_2\geq x$ which implies $p_1\geq \beta_0(a)$ or $p_2\geq \beta_0(a)$ again. Applying the
 induction assumption on term complexity, we find $q_1 \geq \beta_{N}(a)$ or $q_2 \geq \beta_{N}(a)$.
 Either way $q\geq \beta_{N}(a)$ and the base case is proved.
 
 Assume $k>0$ and $p\geq\beta_k(a)$ in the following.
 By definition $\beta_k(a) = \bigwedge W$ for $W := \{ w\in H^\vee_{k-1}\ :\ h(w) \geq a \}$.
 By Whitman's condition $p_1\vee p_2 \geq \bigwedge W$ yields $p_1 \geq \beta_k(a)$ or $p_2 \geq \beta_k(a)$ or
 $p\geq w$ for some $w\in W$. The first two cases are again straightforward using the induction assumption
 on term complexity. For the third case note that $w = \bigvee U$ for some $U\subseteq H_{k-1}$.
 For each $u\in U$, we have $p\geq u$ and $u\geq \beta_{k-1}(h(u))$, which yield $q\geq\beta_{k-1+N}(h(u))$
 by the induction assumption on $k$. Hence
\begin{align*}
 q &\geq \bigvee \{ \beta_{k-1+N}(h(u)) \sts u\in U \} \\
   & \geq \beta_{k+N} (\bigvee\{ h(u) \sts u\in U \}) &\text{ by Lemma~\ref{lem:beta}~\eqref{it:Vbk}} \\
  & = \beta_{k+N} (h(w)) \\
  & \geq \beta_{k+N} (a) &\text{ by } h(w) \geq a \text{ and Lemma~\ref{lem:beta}~\eqref{it:mon}}. 
\end{align*}
\end{proof}  


\begin{corollary}
 Let $X$ be a finite set, $L$ a finite lattice, and $h\colon F(X)\to L$ an epimorphism.

 Then $\ker h$ is finitely generated as a sublattice of $F(X)^2$ iff $h$ (equivalently $L$) is lower and upper bounded.
\end{corollary}
...
\end{theorem}
 ...
 In the following lemma we distinguish between lattice terms $t$ over $X$ in the language $\wedge,\vee$
 and the elements $\bar{t}\in\alg F (X)$ which they induce. 

\begin{lem}
 Let $p=\bigvee P$ be a lattice term of minimal size such that $\bar{p}\in\bar{H}_1$ and all elements in $P$
 are meets of subterms or variables in $X$. If $|P|>1$, then $\bar{P} \subseteq \bar{H}_0$.
\end{lem}

\begin{proof}
 Assume $|P|>1$. Let $r := \bigwedge_i (\bigvee R_i)\in H_1$ for sets $R_i\subseteq H_0$ such that $\bar{r}=\bar{p}$.
 First we will show that $r$ can actually be chosen as join of elements in $H_0$.
  
 By Whitman's condition $\bigwedge_i (\bigvee R_i) \leq \bigvee P$ yields $r\leq t$ for some $t\in P$ or
 $\bigvee R_i \leq p$ for some $i$.
 In the former case $r\leq t\leq p$ yields $\bar{r} = \bar{t} =\bar{p}$ which contradicts the minimality of $p$.
 Hence we have the latter and $r \leq \bigvee R_i \leq p$ yields $\bigvee \bar{R}_i = \bar{p}$ for some $i$.
 We may choose $r = \bigvee R_i$ with $R_i\subseteq H_0$.
 In fact redefining the sets $R_i$ we may assume
\[ r = \bigvee_i (\bigwedge R_i) \text{ for } R_i \subseteq X. \]  
 From $r \leq p$ it follows that $\bigwedge R_i\leq \bigvee P$ for all $i$. Fix $i$.
 By Whitman's condition either $\bigwedge R_i\leq t$ for some $t\in P$ or $x\leq \bigvee P$ for some $x\in R_i$.
 The latter case yields $x\leq t$ for some $t\in P$. Hence either way it follows that
\begin{equation}  \label{eq:Rit}
 \forall i \exists t\in P\colon \bigwedge R_i\leq t. 
\end{equation}
 Conversely we claim
\begin{equation} \label{eq:tRi}
  \forall t\in P \exists i\colon t\leq \bigwedge R_i.
\end{equation}
 For the proof let $t\in P$ and consider $p\leq r$. Then $t\leq \bigvee_i (\bigwedge R_i)$.
 By the assumption of the lemma either $t$ is a variable or a proper meet.
 If $t\in X$, then $t\leq \bigwedge R_i$ for some $i$ follows since variables are join prime.
 So assume $t = \bigwedge T$ for $|T|>1$. 
 By Whitman's condition either $t\leq \bigwedge R_i$  for some $i$ or $s\leq r$ for some $s\in T$.
 Suppose the latter. Note that $p = (\bigvee P\setminus\{t\}) \vee \bigwedge T$ and
 $\bigwedge T\leq s \leq r$. Hence $\overline{(\bigvee P\setminus\{t\}) \vee s} = \bar{r}$. Since $|T|>1$,
 this contradicts the minimality of $p$. Thus~\eqref{eq:tRi} is proved.

 Let $t\in P$. By~\eqref{eq:tRi} and~\eqref{eq:Rit} we have $i$ and $s\in P$ such that
 $t\leq \bigwedge R_i\leq s$.
 If $s\neq t$, then $\bigvee\bar{P} = \bigvee\overline{P\setminus\{t\}}$ contradicts the minimality of $p$.
 Hence $s=t$ which yields $\bar{t}=\bigwedge\bar{R}_i$. Thus $\bar{P}\subseteq\bar{H_0}$. 
\end{proof}
