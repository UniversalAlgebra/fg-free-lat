...
classes of $\ker h$ do not contain generators of $\mathbf F$ we brought to our attention by J.B.~Nation.  Despite the fact that the unbounded kernel classes do not contain generators, we are still able to prove that the kernel is not a finitely generated subalgebra of $\mathbf F \times \mathbf F$.
\begin{proposition}
  \label{prop:2.2}
...
\end{claim}
\begin{proof}
  Assume (the induction hypothesis) that the
  implications~(\ref{eq:acc01})--(\ref{eq:acc04}) hold for both 
  $(p, q) = (p_1, q_1)$ and for $(p, q) = (p_2, q_2)$.
