...
\begin{itemize}
\item for $i \in \mathbb N$,  let $m_i = (x_i \wedge y_i) \vee (x_i \wedge z_i)\vee (y_i \wedge z_i)$, where
\item $x_0 = x$, $y_0 = y$, $z_0 = z$, and for $i \in \mathbb N-\{0\}$,
    \[x_{i+1} = x_i\vee m_i, \quad y_{i+1} = y_i\vee m_i,\quad z_{i+1} = z_i\vee m_i.\]
\end{itemize}
\[x_{i+1} = x_i\vee m_i = x_{i-1} \vee m_{i-1} \vee m_i = \cdots = x \vee m_0 \vee \cdots \vee m_i = x \vee m_i.\]
\noindent Notice that 
$x_{i+1} = x_i\vee (y_i \wedge z_i)$ (similarly for $y_i$ and $z_i$), so we have
\begin{align*}
...
x_2 &= x \vee (y \wedge z) \vee \bigl\{\bigl[y \vee (x \wedge z)\bigr] \wedge \bigl[z \vee (x \wedge y)\bigr]\bigr\},\\  
& \vdots\\
x_{i+1} &= x_i \vee (y_i \wedge z_i), \quad y_{i+1} = y_i\vee (x_i \wedge z_i), \quad z_{i+1} = z_i\vee (x_i \wedge y_i).
\end{align*}
Notice also that 
$y \wedge z \leq y \leq y \vee (x \wedge z)$ and 
$y \wedge z \leq z \leq z \vee (x \wedge y)$.  Therefore,   
\[
x_2 = x \vee \bigl\{\bigl[y \vee (x \wedge z)\bigr] \wedge \bigl[z \vee (x \wedge y)\bigr]\bigr\}. 
\]
In fact,
\[
x_{i+1} = x_i \vee (y_i \wedge z_i) = x \vee (y_i \wedge z_i).
\]
\begin{align*}
x_{i+1} &= x_i \vee (y_i \wedge z_i) \\
&= \bigl[x_{i-1} \vee (y_{i-1} \wedge z_{i-1})\bigr]   \vee (y_i \wedge z_i) \\
&= x_{i-1} \vee (y_i \wedge z_i)
\end{align*}
since  $y_{i-1} \wedge z_{i-1} \leq y_i \wedge z_i$.
...
\begin{itemize}
  ...
  {\it Proof of Subclaim~\ref{claim:3.2.1}.}    The first item is obvious; for the second, if $m_{x,n}\geqslant y$, then $m_{x,n}\wedge y = y$, and 
    ...
    Recall, $m_{x, n+1} = m_{x,n} \vee (m_{y,n} \wedge m_{z,n})$, so $m_{x,n+1} > m_{x,n}$
    holds as long as $m_{x,n} \ngeqslant m_{y,n} \wedge m_{z,n}$.
