...
\item[{\bf Ex 4.}] Let $n > 2$, $X = \{x_0, x_1,\dots, x_{n-1}\}$, and $\mathbf{L} = \mathbf{2} \times \mathbf{2}$.  
Let $h\colon \mathbf{F} \twoheadrightarrow \mathbf{L}$ be an epimorphism. Then $K = \operatorname{ker}h$ is finitely generated.  
\end{itemize}

\medskip

\noindent {\it Proof of Ex 4.} Let the universe of $\mathbf{L} = \mathbf{2} \times \mathbf{2}$ be $\{0, a, b, 1\}$, where $a\vee b = 1$ and $a\wedge b = 0$.  
...
\noindent {\it Claim 4.1.} $h^{-1}\{a\}$ has least and greatest elements, namely $\ell_a = \bigwedge (X_a \cup X_1)$ and $g_a = \bigvee (X_a\cup X_0)$.  (Similarly, 
$h^{-1}\{b\}$ has least and greatest elements, $\ell_b$ and $g_b$.)

\bigskip


\noindent {\it Claim 4.2.} $h^{-1}\{0\}$ has least and greatest elements, 
namely, $\ell_0 = \bigwedge X$ and $g_0 = g_a \wedge g_b$.

\bigskip

\noindent {\it Claim 4.3.} $h^{-1}\{1\}$ has least and greatest elements, namely $\ell_1 = \ell_a \vee \ell_b$ and $g_1 = \bigvee X$.

\medskip

\noindent {\it Proof of Claim 4.1.}
...
\medskip
\noindent {\it Claim 4.} If $r \in F$ and $h(r) = p$, then $(r, \ell_p), (r, g_p) \in \langle Y \rangle$.
