...
$q_2 \leq \alpha_N(b)$ and, in either case, we could conclude that 
$q_1 \wedge q_2 \leq \alpha_N(b)$. Unfortunately, we do not know that 
either $p_1\leq x_1$ or $p_2\leq x_1$, since $x_1$ is not meet prime.

To say that $x_1$ is meet prime means that \emph{for all} $p$ and $q$ with $p \wedge q \leq x_1$,
we have either $p \leq x_1$ or $q \leq x_1$.  But this is actually more that we need.
For our purposes, we only require that the condition
\begin{equation}
p \wedge q \leq x_1 \quad \Longrightarrow \quad p \leq x_1 \text{ or } q \leq x_1,
\end{equation}
is satisfied for some special elements $p$ and $q$, namely, those that form pairs
$(p, q)$ belonging to $\<K \>$.
