...
As above, let $x_1 = x \vee (y \wedge z)$
and define $a = 0\vee (1 \wedge 2)$,  
so that $h(x_1) = a$. 
Suppose $K \subseteq \mathbf F \times \mathbf F$ is a finite set. 
We wish to prove $\<K\> \neq \ker h$.
First, we show there exists $N < \omega$ such that 
for all $(p, q) \in \<K\>$ in the same kernel class as $x_1$
(that is, $h(p) = a = h(q)$) %$(p, q)\in \mathbf F \times \mathbf F$ %with $h(p) = b = h(q)$, 
the following implication holds:
\begin{equation}
\text{ if $p \leq x_1$, then $q \leq \alpha_N(a)$;} \label{eq:acc1}
\end{equation}
% \text{ if $p \leq y_1$, then $q \leq \alpha_N(b)$;} \label{eq:acc2}\\ \text{ if $p \leq z_1$, then $q \leq \alpha_N(c)$.}\label{eq:acc3} \end{gather}
Since $\alpha_n(a)$
% , $\alpha_n(b)$, and $\alpha_n(c)$ are infinite ascending chains, 
is an infinite ascending chain, 
we can certainly find $N < \omega$ such that~(\ref{eq:acc1}) holds for all 
of the (finitely many) pairs in $K$.

Now suppose $(p, q) = (p_1, q_1) \wedge (p_2, q_2) \in \<K\>$ and $h(p) = a = h(q)$. 
Then $p_1$, $p_2$, $q_1$, $q_2$ must all belong to $h^{-1}\{a\}$ as well. 
Indeed, $a = h(p) = h(p_1 \wedge p_2)= h(p_1) \wedge h(p_2)$, and
$a = 0\vee (1 \wedge 2)$ is meet irreducible, so 
$h(p_1) = a = h(p_2)$. Similarly for $q_1$ and $q_2$.

Assume $p = p_1\wedge p_2 \leq x_1$.  
We must show $q = q_1 \wedge q_2 \leq \alpha_N(a)$.
Assume the induction hypothesis that $(p_1, q_1)$
and $(p_2, q_2)$ both satisfy~(\ref{eq:acc1}). 
Then, since $p_1\wedge p_2 \leq x \vee (y \wedge z)$, condition (W+) implies
that one of the following must hold:
\begin{enumerate}
\item   $p_1\leq x \vee (y \wedge z)$;  
\item   $p_2 \leq x \vee (y \wedge z)$;  
\item   $p_1\wedge p_2 \leq y \wedge z$.  
\end{enumerate}
Case~(3) is impossible. Indeed, $p_1\wedge p_2 \leq y$ in that case, and  
$y$ is meet prime, so either $p_1\leq y$ or $p_2\leq y$.
But if $p_1 \leq y$, then $h(p_1) \leq h(y) = 1$, and we have already established that,
under present assumptions, $h(p_1) = 0 \vee (1 \wedge 2)$, which is not below $1$.
Of course, a similar contradiction is reached if $p_2 \leq y$.

In Case~(1), the induction hypothesis yields $q_1 \leq \alpha_N(a)$.
Similarly, in Case~(2) we have $q_2 \leq \alpha_N(a)$ by the induction hypothesis.
In either case, we have $q = q_1 \wedge q_2 \leq \alpha_N(a)$, which is the desired result.

From what we have shown, it is easy to conclude that 
$\<K\> \neq \ker h$. Indeed, the pair $(p, q) = (x_1, \alpha_{N+1}(a))$ 
is in $\ker h$, as $h(x_1) = a = h(\alpha_{N+1}(a))$, but does not satisfy 
condition~\ref{eq:acc1}---a condition that, as we just proved, all pairs in $\<K\>$ must satisfy.
\qed
