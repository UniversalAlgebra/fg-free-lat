...
% x_1 \wedge z_1 \leq  (x \wedge y) \vee (x \wedge z) \vee (y \wedge z),\\
% y_1 \wedge z_1 \leq  (x \wedge y) \vee (x \wedge z) \vee (y \wedge z).
% \end{gather}
\[
x_1 \wedge y_1 \leq  (x \wedge y) \vee (x \wedge z) \vee (y \wedge z).
\]
...
Notice that the example above involves an infinite ascending chain $\{m_n\} \subseteq \mathbf F$
such that $h(m_1) = h(m_2) = \cdots$; thus, for $n\geq 1$, all $m_n$ belong to the same 
(unbounded) class of $\ker h$, and this class contains 
$m_0 = (x \wedge y) \vee (x \wedge z) \vee (y\wedge z)$.
%$m_0 = (x_i \wedge y_i) \vee (x_i \wedge z_i) \vee (y_i \wedge z_i).
% As above, let 
% $m_0 = (x_i \wedge y_i) \vee (x_i \wedge z_i) \vee (y_i \wedge z_i).
Define $b = (0\wedge 1) \vee (0 \wedge 2)\vee (1 \wedge 2)$.  
Then $h(m_1) = b$. 
...
\text{ if $(p, q) \in \<K\>$ and $p \leq m_0$, then $q \leq \alpha_N(b)$.} 
...
$b = (0\wedge 1) \vee (0 \wedge 2)\vee (1 \wedge 2)$ is meet irreducible, so 
$h(p_1) = b = h(p_2)$. Similarly for $q_1$ and $q_2$.

Assume $(p, q) \in \<K\>$ and $p = p_1\wedge p_2 \leq m_0$.  
We must show $q_1 \wedge q_2 \leq \alpha_N(b)$.
Assume the induction hypothesis that $(p_i, q_i)$ satisfies~(\ref{eq:dcc}) for both $i = 1, 2$.
The assumption that $p = p_1\wedge p_2 \leq m_0$ is expanded as follows:  
\[
p_1 \wedge p_2 \leq (x \wedge y) \vee (x \wedge z) \vee (y\wedge z).
\]
Whitman's Theorem implies that one of the following must hold:
\begin{enumerate}
\item $p_1 \wedge p_2 \leq x \wedge y$.
\item $p_1 \wedge p_2 \leq x \wedge z$.
\item $p_1 \wedge p_2 \leq y\wedge z$.
\item $p_1 \leq (x \wedge y) \vee (x \wedge z) \vee (y\wedge z)$.
\item $p_2 \leq (x \wedge y) \vee (x \wedge z) \vee (y\wedge z)$.
\end{enumerate}
In Cases (4) and (5) we have either $p_1\leq m_0$ or $p_2\leq m_0$, so 
$q_1 \leq \alpha_N(b)$ or 
$q_2 \leq \alpha_N(b)$, by the induction hypothesis; in either case, we 
conclude that $q_1 \wedge q_2 \leq \alpha_N(b)$, as desired. 
So we try to rule out the remaining cases.  

In Case (1), we have $p_1 \wedge p_2 \leq x \wedge y$, so in particular 
$p_1 \wedge p_2 \leq x$. As $x$ is meet prime, we have $p_1 \leq x$ or $p_2 \leq x$.
Also, 
$p_1 \wedge p_2 \leq y$, so $p_1 \leq y$ or $p_2 \leq y$ 

\medskip

In Case (2), we have $p_1 \wedge p_2 \leq x \wedge z$.

\medskip

In Case (3), we have $p_1 \wedge p_2 \leq y\wedge z$.

\bigskip

\wjd{Complete the proof of Prop 2.2}

\bigskip

\noindent {\bf More scratch work.}
\medskip
...
Our working assumption is that $p_1\wedge p_2 \leq x \vee (y \wedge z)$. This and (W+) imply
...
\item   $p_1\wedge p_2 \leq y \wedge z$.  
\end{enumerate}
In cases (1) and (2), the induction hypothesis gives us the desired result.
% In case (3), since $x$ is meet prime, we have either $p_1 \leq x$ or $p_2\leq x$,
% and so either $p_1 \leq x_1$ or $p_2\leq x_1$; again, the induction hypothesis 
% yields the desired result in this case.
In case (3), we have $p_1\wedge p_2 \leq y$ and $p_1\wedge p_2 \leq z$. 
