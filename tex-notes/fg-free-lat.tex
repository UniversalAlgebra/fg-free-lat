...
\end{conjecture}
We know how to prove this conjecture in the following special case 
(see Appendix~\ref{app:conjecture} for the proof): 
if $h$ is unbounded then there is a class of 
$\ker h$ that contains an infinite descending chain as well as a join prime element of 
$\mathbf F$. Unfortunately, we don't yet know how to prove that there will always be 
a join prime element in one of the ``infinite'' classes of $\ker h$ 
(i.e., a class containing an infinite chain).

J.B.~Nation brought the next result to our attention.  
It gives an example in which the infinite 
