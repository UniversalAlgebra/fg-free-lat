...
However, the argument is quite tedious and similar to those above, so we relegate it to 
Appendix Section~\ref{app:m-seq-proof}.
\end{proof}
...
We now prove that, indeed, $a\wedge b \leq c$, and form this follows both $b \wedge c\leq a$ and $a \wedge c\leq b$, by symmetry. Recall that 
...
$a \wedge b = \bigl\{0\wedge \bigl[1\vee (0 \wedge 2)\bigr]\bigr\} \vee (1 \wedge 2)$,
and since $0 \geq 0 \wedge 2$, modularity also implies
\[
...
From what we just proved, not only do we have $d = a \wedge c$, but also $d = a\wedge b = a\wedge c = b \wedge c$.

Now, suppose $K \subseteq \mathbf F \times \mathbf F$ is a finite set. 
We wish to prove $\<K\> \neq \ker h$.
...
is an infinite ascending chain, so we can certainly find, for each of the four implications above, 
a number $N_w < \omega$ such that the given implication holds for $N = N_w$ and
for all of the (finitely many) pairs in $K$.  Therefore, 
all four implications will hold for $(p, q) \in K$ if we take $N = \max \{N_a, N_b, N_c, N_d\}$. In fact, we will now see that the same $N$ works for all $(p, q) \in \<K\>$.

\begin{claim}\label{claim:wedge} If $(p, q) = (p_1, q_1) \wedge (p_2, q_2) \in \<K\>$ and $p = p_1\wedge p_2 \leq m_1$, then $q = q_1 \wedge q_2 \leq m_N$.
\end{claim}
\begin{proof}
  Assume the following induction hypothesis: the implications~(\ref{eq:acc01})--(\ref{eq:acc04}) hold for
  $(p, q) = (p_1, q_1)$ and for $(p, q) = (p_2, q_2)$.

  Since $p_1\wedge p_2 \leq m_1 = (x_1 \wedge y_1) \vee (x_1 \wedge z_1) \vee (y_1 \wedge z_1)$, condition (W+) implies
  ...
  $\{1, 2\}$ for which $p_i \leq m_1$ holds, then $q_i \leq m_N$ also holds by the induction hypothesis; therefore, 
  $q = q_1 \wedge q_2 \leq q_i \leq m_N$, which is the desired result.

  To complete the proof of Claim~\ref{claim:wedge}, it suffices to handle Case~(3) since Cases~(4) and~(5) submit to the same argument, by symmetry.
  In Case~(3), $p_1\wedge p_2 \leq x_1 \wedge y_1$. In particular, 
  $p_1\wedge p_2 \leq x_1$ and $p_1\wedge p_2 \leq y_1$. 
  Considering the first of these, $p_1\wedge p_2 \leq x_1 = x \vee (y \wedge z)$, and Whitman's condition, 
  ...
  \end{enumerate}
  Considering the second, $p_1\wedge p_2 \leq y_1 = y \vee (x \wedge z)$, one of the following holds:
  \begin{enumerate}[(i)]
    \item   $p_1\leq y_1$;
    \item   $p_2 \leq y_1$;
    \item   $p_1\wedge p_2 \leq x \wedge z$.  
  \end{enumerate}
  Suppose~(a) and~(i) hold; that is, $p_1\leq x_1$ and $p_1 \leq y_1$.
  Then, by the induction hypothesis, $q_1 \leq x_N$ and 
  $q_1 \leq y_N$, so $q = q_1\wedge q_2 \leq x_N \wedge y_N\leq m_N$, which is the desired conclusion.
  Of course, the same argument handles the case in which both (b) and (ii) hold. 

  Next, suppose we are in case (a) $\wedge$ (ii), so that both $p_1\leq x_1$ and $p_2\leq y_1$ hold.  Then by the induction hypothesis, $q_1 \leq x_N$ and 
  $q_2 \leq y_N$, so $q = q_1\wedge q_2 \leq x_N \wedge y_N \leq m_N$, as desired.

  It remains to consider the case (a) $\wedge$ (iii) and the case (c) $\wedge$ (iii). (The other cases are handled analogously by symmetry.)
  Suppose (a) $\wedge$ (iii). Then $p_1 \leq x_1$ so $q_1 \leq x_N$, so $q\leq
  x_N$. Also, (iii) and (W+) imply either $p_1 \leq x\wedge z$ or 
  $p_2 \leq x\wedge z$.  Without loss of generality, assume 
  $p_1 \leq x\wedge z$.  Then $p_1 \leq y \vee (x \wedge z) = y_1$, so 
  $q_1 \leq y_N$.  Therefore, $q = q_1 \wedge q_2 \leq x_N \wedge y_N \leq m_N$,
  as desired.

  If (c) $\wedge$ (iii), then  by (c)
  and (W+) either $p_1\leq y \wedge z$ or $p_2 \leq y \wedge z$.
  Assuming $p_1\leq y \wedge z \leq x_1$, we have $q_1\leq x_N$.
  By (iii) and (W+) either $p_1\leq x\wedge z$ or $p_2 \leq x\wedge z$, so either $q_1\leq y\vee (x\wedge z) = y_1$, or 
  $q_2\leq y\vee (x\wedge z) = y_1$. Either way, we have $q = q_1\wedge q_2 \leq y_N$.  Combining these observations yields $q\leq x_N \wedge y_N \leq m_N$, as desired.
  
  This completes the proof of Claim~\ref{claim:wedge}.
\end{proof}

\begin{claim}\label{claim:vee}
  If $(p, q) = (p_1, q_1) \vee (p_2, q_2) \in \<K\>$ and
  $p = p_1\vee p_2 \leq m_1$, then $q = q_1 \vee q_2 \leq m_N$.
\end{claim}
\begin{proof}
  Assume (the induction hypothesis) that the implications~(\ref{eq:acc01})--(\ref{eq:acc04}) hold for both $(p, q) = (p_1, q_1)$ and for $(p, q) = (p_2, q_2)$.
  Since $p_1\vee p_2 \leq m_1$, we have $p_1 \leq m_1$ and $p_2\leq m_1$.
  Therefore, the induction hypothesis implies that $q_1 \leq m_N$ and 
  $q_2 \leq m_N$ hold, so $q = q_1 \vee q_2 \leq m_N$, as desired.
\end{proof}

  It follows from Claims~\ref{claim:wedge} and~\ref{claim:vee} that 
  $\<K\> \neq \ker h$. Indeed, since $\{m_n\}$ is a strictly increasing sequence, $m_N < m_{N+1}$, so the pair $(p, q) = (m_1, m_{N+1})$ belongs to $\ker h$, as $h(m_1) = d = h(m_{N+1})$, but does not satisfy condition~(\ref{eq:acc04}). On the other hand, we just proved that all pairs in $\<K\>$ satisfy~(\ref{eq:acc01})--(\ref{eq:acc04}) when $K$ is a finite set.   The proof of Proposition~\ref{prop:2.2} is now complete.\qed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
  ...
  \appendix
\section{Proof that $\{m_k\}$ in Lemma~\ref{claim:2.3} is strictly increasing}
\label{app:m-seq-proof}.
