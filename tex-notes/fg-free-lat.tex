...
\def\OPTinnermargin{1.25in}    % Margin on the inner side of the page
\def\OPTbindingoffset{0in} % Extra offset on the inner side
%% \def\OPToutermargin{0.75in}   % Margin on the outer side of the page
\def\OPToutermargin{1.25in}   % Margin on the outer side of the page
...
The notation and definitions we use follow those of Freese, Jezek, 
Nation~\cite{MR1319815}, although they have their origins in the earlier work 
of Ralph McKenzie (reference needed).  

We define a pair of closure operators, 
...
\]
We adopt the following convention: if $\mathbf L$ has a greatest element 
...
subset of $K$, and $\bigcup H_n = K$, since $X$  generates $\mathbf X$.

Let $h \colon \mathbf K \to \mathbf L$ be a lattice epimorphism and define, 
...
\end{conjecture}
If we could assume that whenever $h$ is unbounded then there is a class of 
$\ker h$ containing both an infinite chain and a generator of $\mathbf F$, 
then there is a straightforward proof of the conjecture.
(See the Appendix for details.)  Unfortunately, as the next result shows, this 
assumption is not always valid.

\begin{proposition}
Let $\mathbf{F} = \mathbf{F}(x,y,z)$, and let $\mathbf{L} = \mathbf{F}_{\mathbf{M}_3}(3)$ (see Figure~\ref{fig:1}).  
...
\begin{claim}
  If $\{s_n\}$ is any one of the four sequences just defined, then for $n>0$, we have $s_{n+1} > s_n$ and $h(s_{n+1}) = h(s_n)$.
\end{claim}

\wjd{To do: fill in proof!}
...
\end{itemize}
This completes the proof of Subclaim~\ref{claim:2.1}.

It follows from the subclaim just proved and Lemma~\ref{lem:1} that 
$(m_{x, M+1}, x)\in K \setminus \langle X \rangle$, so the proof of the proposition is complete.
\end{proof}
...
\begin{itemize}
  ...
  \begin{subclaim}\label{claim:3.2.1}For all $n\in \mathbb N$,
    ...
    \end{enumerate}
  \end{subclaim}
  {\it Proof of Subclaim~\ref{claim:3.2.1}.}
    The first item is obvious; for the second, if $m_{x,n}\geqslant y$, then $m_{x,n}\wedge y = y$, and 
...
Suppose $r = s \wedge t$.
  ...
  \end{itemize}
Similarly, in each of these three subcases we have $(r, g_p) \in \langle Y \rangle$.\\[6pt]
Suppose $r = s \vee t$.
  \begin{itemize}
...
\bigskip
  ...
  \appendix
\section{Proof of Conjecture under special assumptions}
\begin{prop}
Suppose $\mathbf L = \langle L, \wedge, \vee\rangle$ is a finite lattice and 
$h\colon \mathbf{F} \twoheadrightarrow \mathbf{L}$ a lattice epimorphism.
Suppose also that whenever $h$ is unbounded then there is a class of 
$\ker h$ containing both an infinite chain and a generator of $\mathbf F$. 
Then $h$ is bounded whenever its kernel is a finitely generated sublattice 
of $\mathbf F \times \mathbf F$.
\end{prop}
\begin{Proof}\noindent Suppose $h$ is not lower bounded. Then by Fact~\ref{fact:1} 
there is an element $y_0\in L$ such that 
$\beta_0(y_0) > \beta_1(y_0) > \cdots$ is an infinite descending chain. 
