...
% \newcommand\hx{\ensuremath{\bigl[0 \vee (1 \wedge 2)\bigr]}}
% \newcommand\hy{\ensuremath{\bigl[1 \vee (0 \wedge 2)\bigr]}}
% \newcommand\hz{\ensuremath{\bigl[2 \vee (0 \wedge 1)\bigr]}}
...
%&= \bigl\{\hx \wedge \hy\bigr\} \vee \bigl\{\hx \wedge \hz\bigr\}\vee \bigl\{\hy \wedge \hz\bigr\}
\end{align*}
...
% \[
% h(x_1) \wedge h(y_1) = h(x_1) \wedge h(z_1) = h(y_1) \wedge h(z_1). %= (0\wedge 1) \vee (0\wedge 2) \vee (1\wedge 2).
% \]
% h(m_n) = (x_n \wedge y_n) \vee (x_n \wedge z_n) \vee (y_n \wedge z_n);\\
...
Define $x_0 = x \vee (y \wedge z)$ and $y_0 = 0\vee (1 \wedge 2)$.  
In particular, $h(x_0) = y_0$. 
Suppose $p_1 \wedge p_2 \leq x_0$.  Does this imply that either 
$p_1 \leq x_0$ or $p_2 \leq x_0$?  No, because we could have $p_1 = y$ and $p_2 = z$.

Suppose $K \subseteq \mathbf F \times \mathbf F$ is a finite set. 
We will prove $\<K\> \neq \ker h$.
First, we show there exists $N < \omega$ such that 
for all $(p, q)\in \mathbf F \times \mathbf F$ with $h(p) = y_0 = h(q)$, 
the following implication holds:
\begin{equation}\label{eq:dcc}
\text{ if $(p, q) \in \<K\>$ and $p \leq x_0$, then $q \leq \alpha_N(y_0)$.} 
%, \; \text{ for all $(p, q) \in \<K\>\cap h^{-1}(y_0)^2$.}
\end{equation}
Since $\alpha_n(y_0)$ is an infinite ascending chain, 
...
Now suppose $(p, q) = (p_1, q_1) \wedge (p_2, q_2)$ and $h(p) = y_0 = h(q)$. 
Then $p_1$, $p_2$, $q_1$, $q_2$ must all belong to $h^{-1}\{y_0\}$ as well. 
Indeed, $y_0 = h(p) = h(p_1 \wedge p_2)= h(p_1) \wedge h(p_2)$, and
$y_0 = 0\vee (1 \wedge 2)$ is meet irreducible, so 
$h(p_1) = y_0 = h(p_2)$. Similarly for $q_1$ and $q_2$.

Assume $(p, q) \in \<K\>$ and $p = p_1\wedge p_2 \leq x_0$.  
We must show $q_1 \wedge q_2 \leq \alpha_N(y_0)$.
Assume the induction hypothesis that $(p_i, q_i)$ satisfies~(\ref{eq:dcc}) for both $i = 1, 2$.

How do we know that either $p_1\leq x_0$ or $p_2\leq x_0$? We don't...  

...maybe we should try to exploit a $D^d$-cycle here, involving
\[p_1 \wedge (p_1 \to x_0) \leq x_0\]
We know that the maximum element (which is denoted here by $p_1 \to x_0$), 
among those that meet with $p_1$ below $x_0$, exists by the existence of a 
$D^d$-cycle. 
...
\begin{theorem}[\protect{\cite[Theorem 2.1]{MR1319815}}]
...
Note that because $K$ is finitely generated, it has a greatest element
...
\begin{theorem}[\protect{\cite[Theorem 2.2]{MR1319815}}]\label{thm:2.2}
   Let $K$ be finitely generated, and let $h : K \to L$ be a lattice
  homomorphism. For each $a \in L$ with $a \leq h(1_{\mathbf K})$, we have
\begin{enumerate}
...
A \textbf{join cover} of the element $a \in L$ is any finite
subset $S \subseteq L$ such that $a < \bigvee S$. 
A join cover $S$ of $a$ is \textit{nontrivial} if $a \nleq s$ for all
$s \in S$. Let $\mathcal C (a)$ denote the set of all nontrivial join 
covers of $a$ in $L$.

\begin{theorem}[\protect{\cite[Theorem 2.3]{MR1319815}}]\label{thm:2.3}
Let $\mathbf K$ be generated by the finite set $X$, and let 
$h : K \to L$ be a lattice homomorphism. 
...
exclude if $L$ satisfies a weak finiteness condition, as we now explain.

Recall, for finite subsets $A$, $B \subseteq L$ we say that $A$ join refines
$B$, written $A \ll B$, if for every $a \in A$ there exists $b \in B$ 
...
$S \subseteq T$. This technical formulation is equivalent to our more 
...
\item \textit{whenever any element of $S$ is deleted or replaced by a (finite) 
...
Let us say that $L$ has the \textbf{minimal join cover refinement property} if for each
$a \in L$, $\mathcal M(a)$ is finite and every nontrivial join cover of a refines to a minimal
one. Clearly every finite lattice has the minimal join cover refinement property,
but so do free lattices.

The following reformulation of Theorem~\ref{thm:2.3} is obvious
but simplifies the calculation of $\beta_k$ whenever the minimal join cover 
refinement property holds.

\begin{theorem}[\protect{\cite[Theorem 2.4]{MR1319815}}]\label{thm:2.4}
Let $\mathbf K$ be generated by the finite set $X$, and let 
$h \colon K \to L$ be a lattice homomorphism. 
If L has the minimal join cover refinement property, then
for each $a \in L$ with $a \leq h(1_{\mathbf K})$ and $k\in \omega$, we have
\begin{align*}
    \beta_0(a) &= \bigwedge \{x \in X \mid h (x) \geq a\},\\
    \beta_{k+1}(a) &= \beta_0(a) \wedge 
...
Now let us look for a condition on the lattice $L$ which will insure that the
homomorphism $h \colon K \to L$ is lower bounded. It is clear from 
Theorem~\ref{thm:2.2} that this will happen if and only if for each 
$a \leq h(1_{\mathbf K})$ there exists an $N \in \omega$ such
that $\beta_n(a) = \beta_N(a)$ for all $n \geq N$. In this case, of course, 
$\beta(a) = \beta_N(a)$ for all $a\in \dom \beta = \llb 0, h(1_{\mathbf K})\rrb$, 
where $N$ depends upon $a$.

With this in mind, let 
\begin{itemize}
\item $D_0(L) = $ the set of all join prime elements of $L$, 
i.e., the set of all elements which have no nontrivial join cover. 
\item Given $D_k(L)$, define $D_{k+1}(L) = $ the set of all $p \in L$ 
such that every nontrivial join cover of $p$ refines to a join cover 
contained in $D_k(L)$, i.e., $p \leq \bigvee S$ nontrivially implies 
there exists $T \ll S$ with $p \leq \bigvee T$ and $T \subseteq D_k(L)$. 
\end{itemize}
Note that if $L$ has the minimal join cover refinement property, then 
$p \in D_{k+1}(L)$ if and only if every minimal nontrivial join
cover of $p$ is contained in $D_k(L)$.

The definition clearly implies $D_0(L) \subseteq D_1(L) \subseteq D_2(L) \subseteq \cdots$.  
Let $D(L) = \bigcup D_i$. For $a \in D(L)$, define the $D$-\textbf{rank}, $\rho(a)$, 
to be the least integer $N$ such that $a \in D_N(L)$; for $a \notin D(L)$, 
$\rho (a)$ is undefined. 
The duals of $D_k(L)$, $D_k(L)$, and $\rho(a)$ are denoted by $D^d_k(L)$, 
$D^d(L)$, and $\rho^d(a)$, respectively.
